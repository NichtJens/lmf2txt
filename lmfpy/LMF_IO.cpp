...
void LMF_IO::write_times(MyFILE *out_file, time_t _Starttime, time_t _Stoptime) {
    ...
    if (CTime_version_output > 2003) {
        dummy = INT_MIN_ + 10;
        *out_file << dummy;
        dummy = (unsigned __int32) _Starttime;
        *out_file << dummy;
        dummy = 0;
        *out_file << dummy;
        dummy = INT_MIN_ + 10;
        *out_file << dummy;
        dummy = (unsigned __int32) _Stoptime;
        *out_file << dummy;
        dummy = 0;
        *out_file << dummy;
    } else {
...
void WriteCStringLength(MyFILE &out_file, unsigned __int32 nLength) {
    ...
    if (nLength < 255) {
        dummy_uint8 = nLength;
        out_file << dummy_uint8;
    } else if (nLength < 0xfffe) {
        dummy_uint8 = 0xff;
        out_file << dummy_uint8;
        dummy_uint16 = nLength;
        out_file << dummy_uint16;
    } else if (nLength < 0xffffffff) {
        dummy_uint8 = 0xff;
        out_file << dummy_uint8;
        dummy_uint16 = 0xffff;
        out_file << dummy_uint16;
        dummy_uint32 = nLength;
        out_file << dummy_uint32;
    } else {
        dummy_uint8 = 0xff;
        out_file << dummy_uint8;
        dummy_uint16 = 0xffff;
        out_file << dummy_uint16;
        dummy_uint32 = 0xffffffff;
        out_file << dummy_uint32;
        dummy_uint64 = nLength;
        out_file << dummy_uint64;
    }
}
...
/////////////////////////////////////////////////////////////////
{
    ...
    if (number_of_hits) {
        delete[] number_of_hits;
        number_of_hits = 0;
    }

    for (__int32 i = 0; i < 3; ++i) {
        if (TDC8HP.TDC_info[i]) {
            if (TDC8HP.TDC_info[i]->INLCorrection) {
                delete[] TDC8HP.TDC_info[i]->INLCorrection;
                TDC8HP.TDC_info[i]->INLCorrection = 0;
            }
            if (TDC8HP.TDC_info[i]->DNLData) {
                delete[] TDC8HP.TDC_info[i]->DNLData;
                TDC8HP.TDC_info[i]->DNLData = 0;
            }
            if (TDC8HP.TDC_info[i]) {
                delete TDC8HP.TDC_info[i];
                TDC8HP.TDC_info[i] = 0;
            }
        }
    }
    if (ui32buffer) {
        delete[] ui32buffer;
        ui32buffer = 0;
        this->ui32buffer_size = 0;
    }


    if (CCFHistory_strings) {
        for (__int32 i = 0; i < number_of_CCFHistory_strings; ++i) {
            if (CCFHistory_strings[i]) {
                delete CCFHistory_strings[i];
                CCFHistory_strings[i] = 0;
            }
        }
        delete[] CCFHistory_strings;
        CCFHistory_strings = 0;
    }
    if (DAQ_source_strings) {
        for (__int32 i = 0; i < number_of_DAQ_source_strings; ++i) {
            if (DAQ_source_strings[i]) {
                delete DAQ_source_strings[i];
                DAQ_source_strings[i] = 0;
            }
        }
        delete[] DAQ_source_strings;
        DAQ_source_strings = 0;
    }
    if (DAN_source_strings) {
        for (__int32 i = 0; i < number_of_DAN_source_strings; ++i) {
            if (DAN_source_strings[i]) {
                delete DAN_source_strings[i];
                DAN_source_strings[i] = 0;
            }
        }
        delete[] DAN_source_strings;
    ...
    if (CCFHistory_strings_output) {
        ...
        delete[] CCFHistory_strings_output;
        CCFHistory_strings_output = 0;
    }
    if (DAQ_source_strings_output) {
        for (__int32 i = 0; i < number_of_DAQ_source_strings_output; ++i) {
            if (DAQ_source_strings_output[i]) {
                delete DAQ_source_strings_output[i];
                DAQ_source_strings_output[i] = 0;
            }
        }
        delete[] DAQ_source_strings_output;
        DAQ_source_strings_output = 0;
    }
    if (DAN_source_strings_output) {
        for (__int32 i = 0; i < number_of_DAN_source_strings_output; ++i) {
            if (DAN_source_strings_output[i]) {
                delete DAN_source_strings_output[i];
                DAN_source_strings_output[i] = 0;
            }
        }
        delete[] DAN_source_strings_output;
...
//  READ LMF-HEADER
    ...
    *input_lmf >> data_format_in_header;
    ...
    if (data_format_in_header != LM_USERDEF && data_format_in_header != LM_SHORT && data_format_in_header != LM_SLONG &&
        data_format_in_header != LM_DOUBLE && data_format_in_header != LM_CAMAC) {
        errorflag = 8;
        CloseInputLMF();
        return false;
    }
...
//  READ USER-HEADER
    if (Cobold_Header_version >= 2008) {
        *input_lmf >> LMF_Header_version;
        byte_counter += sizeof(__int32);
    }

    if (Cobold_Header_version <= 2002) {
        *input_lmf >> unsigned_int_Dummy;
        byte_counter += sizeof(__int32);
    }
    ...
    if (Cobold_Header_version >= 2008) {
        unsigned __int64 temp;
        *input_lmf >> temp;
        unsigned_int_Dummy = (unsigned __int32) (temp);
    ...
    *input_lmf >> DAQVersion;
    byte_counter += sizeof(__int32);    // Version is always 2nd value

    *input_lmf >> DAQ_ID;
    byte_counter += sizeof(__int32);    // DAQ_ID is always 3ed value
    ...
    if (DAQ_ID == DAQ_ID_TDC8HP || DAQ_ID == DAQ_ID_TDC8HPRAW)
        byte_counter += ReadTDC8HPHeader_LMFV_1_to_7(byte_counter);
    ...
    if (data_format_in_header != LM_USERDEF && data_format_in_userheader != LM_SHORT &&
        data_format_in_userheader != LM_SLONG && data_format_in_userheader != LM_DOUBLE &&
        data_format_in_userheader != LM_CAMAC) {
        errorflag = 6; // error reading header
        CloseInputLMF();
        return false;
    }
...
/////////////////////////////////////////////////////////////////
{
    ...
    *output_lmf << IOaddress;
    byte_counter += sizeof(__int32);        // IO address (parameter 1) always 5th value
    *output_lmf << timestamp_format_output;
    byte_counter += sizeof(__int32);        // TimeInfo (parameter 2) always 6th value  (0,1,2)*32Bit
    ...
    *output_lmf << int_Dummy;
    ...
    if ((DAQVersion_output >= 20020408 && TDC8PCI2.use_normal_method)) {
        *output_lmf << LMF_Version_output;
        byte_counter += sizeof(__int32);
    }

    if (LMF_Version_output >= 0x9) {
        if ((number_of_DAQ_source_strings_output < 0) ||
            (!DAQ_source_strings_output))
            number_of_DAQ_source_strings_output = 0;
        *output_lmf << number_of_DAQ_source_strings_output;
        byte_counter += sizeof(__int32);
    ...
    *output_lmf << system_timeout_output;
    byte_counter += sizeof(__int32);        //   system time-out
    *output_lmf << time_reference_output;
    byte_counter += sizeof(__int32);
    *output_lmf << common_mode_output;
    byte_counter += sizeof(__int32);        //   0 common start    1 common stop
    *output_lmf << tdcresolution_output;
    byte_counter += sizeof(double);        // tdc resolution in ns

    TDCDataType = 1;
    if (DAQVersion_output >= 20020408 && TDC8PCI2.use_normal_method) {
        *output_lmf << TDCDataType;
        byte_counter += sizeof(__int32);
    }
    *output_lmf << timerange;
    byte_counter += sizeof(__int32);    // time range of the tdc in microseconds

    if (DAQVersion_output < 20080507) {
        *output_lmf << number_of_channels_output;
        byte_counter += sizeof(__int32);            // number of channels
        *output_lmf << max_number_of_hits_output;
        byte_counter += sizeof(__int32);            // number of hits
    } else {
        __int64 i64_temp = number_of_channels_output;
        *output_lmf << i64_temp;
        byte_counter += sizeof(__int64);            // number of channels
        i64_temp = max_number_of_hits_output;
        *output_lmf << i64_temp;
        byte_counter += sizeof(__int64);            // number of hits
    }

    *output_lmf << data_format_in_userheader_output;
    byte_counter += sizeof(__int32);                // data format (2=short integer)

    *output_lmf << module_2nd;
    ...
    if (DAQVersion_output >= 20020408 && TDC8PCI2.use_normal_method && (DAQ_ID_output == DAQ_ID_TDC8)) {
        *output_lmf << TDC8PCI2.GateDelay_1st_card;
        byte_counter += sizeof(__int32); // gate delay 1st card
        *output_lmf << TDC8PCI2.OpenTime_1st_card;
        byte_counter += sizeof(__int32); // open time 1st card
        *output_lmf << TDC8PCI2.WriteEmptyEvents_1st_card;
        byte_counter += sizeof(__int32); // write empty events 1st card
        *output_lmf << TDC8PCI2.TriggerFalling_1st_card;
        byte_counter += sizeof(__int32); // trigger falling edge 1st card
        *output_lmf << TDC8PCI2.TriggerRising_1st_card;
        byte_counter += sizeof(__int32); // trigger rising edge 1st card
        *output_lmf << TDC8PCI2.EmptyCounter_1st_card;
        byte_counter += sizeof(__int32); // EmptyCounter 1st card
        *output_lmf << TDC8PCI2.EmptyCounter_since_last_Event_1st_card;
...
/////////////////////////////////////////////////////////////////
{
    ...
    *input_lmf >> IOaddress;
    byte_counter += sizeof(__int32);        // IO address (parameter 1) always 5th value
    *input_lmf >> timestamp_format;
    byte_counter += sizeof(__int32);    // TimeInfo (parameter 2) always 6th value  (0,1,2)*32Bit

    *input_lmf >> DAQ_info_Length;
    byte_counter += sizeof(__int32);        // Length of DAQInfo always 7th value
...
//	TRY
    ...
    *input_lmf >> system_timeout;
    byte_counter += sizeof(__int32);        //   system time-out
    *input_lmf >> time_reference;
    byte_counter += sizeof(__int32);
    *input_lmf >> common_mode;
    byte_counter += sizeof(__int32);            //   0 common start    1 common stop
    *input_lmf >> tdcresolution;
    byte_counter += sizeof(double);    // tdc resolution in ns


    TDCDataType = 1;
    if (DAQVersion >= 20020408 && TDC8PCI2.use_normal_method) {
        *input_lmf >> TDCDataType;
        byte_counter += sizeof(__int32);
    }
    *input_lmf >> timerange;
    byte_counter += sizeof(__int32);            // time range of the tdc in microseconds
    ...
    if (this->DAQVersion < 20080507) {
        *input_lmf >> number_of_channels;
        byte_counter += sizeof(__int32);    // number of channels
        *input_lmf >> max_number_of_hits;
        byte_counter += sizeof(__int32);    // number of hits
    } else {
        __int64 i64_temp;
        *input_lmf >> i64_temp;
        number_of_channels = (unsigned __int32) (i64_temp);
        byte_counter += sizeof(__int64);    // number of channels
        *input_lmf >> i64_temp;
        max_number_of_hits = (unsigned __int32) (i64_temp);
        byte_counter += sizeof(__int64);    // number of hits
    }

    if (int(number_of_channels) > num_channels) {
        errorflag = 19;
        return -100000;
    }
    if (int(max_number_of_hits) > num_ions) {
        errorflag = 20;
        return -100000;
    }
    *input_lmf >> data_format_in_userheader;
    ...
    *input_lmf >> module_2nd;
    ...
    if (DAQVersion >= 20020408 && TDC8PCI2.use_normal_method) {
        *input_lmf >> TDC8PCI2.GateDelay_1st_card;
        byte_counter += sizeof(__int32); // gate delay 1st card
        *input_lmf >> TDC8PCI2.OpenTime_1st_card;
        byte_counter += sizeof(__int32); // open time 1st card
        *input_lmf >> TDC8PCI2.WriteEmptyEvents_1st_card;
        byte_counter += sizeof(__int32); // write empty events 1st card
        *input_lmf >> TDC8PCI2.TriggerFalling_1st_card;
        byte_counter += sizeof(__int32); // trigger falling edge 1st card
        *input_lmf >> TDC8PCI2.TriggerRising_1st_card;
        byte_counter += sizeof(__int32); // trigger rising edge 1st card
        *input_lmf >> TDC8PCI2.EmptyCounter_1st_card;
        byte_counter += sizeof(__int32); // EmptyCounter 1st card
        *input_lmf >> TDC8PCI2.EmptyCounter_since_last_Event_1st_card;
        byte_counter += sizeof(__int32); // Empty Counter since last event 1st card
    }
...
/////////////////////////////////////////////////////////////////
{
    ...
    *input_lmf >> system_timeout;
    ...
    if (LMF_Version >= 9 && DAQVersion >= 20110208) {                // handle new CStringArray information
        ...
        for (unsigned __int32 ui32Count = 0; ui32Count < ui32CStringCount; ui32Count++) {    // now read every CString
            *input_lmf >> iDummy;
            byte_counter += sizeof(__int32);                    // how many characters to read
            for (unsigned __int32 ui32Count2 = 0;
                 ui32Count2 < (unsigned int) iDummy; ui32Count2++)        // read character by character
                input_lmf->read(&cDummy, 1);                            // skip over DAq-source code
            byte_counter += iDummy;
        }
        *input_lmf >> system_timeout;
        byte_counter += sizeof(__int32);        // again system time-out
    }
    ...
    *input_lmf >> timerange;
    ...
    if (DAQVersion >= 20080507 && DAQVersion < 20110208) {
        ...
        *input_lmf >> i64temp;
        number_of_channels = __int32(i64temp);
        byte_counter += sizeof(unsigned __int64);            // number of channels
        *input_lmf >> i64temp;
        max_number_of_hits = __int32(i64temp);
        ...
        *input_lmf >> i64temp;
        number_of_channels2 = __int32(i64temp);
        byte_counter += sizeof(unsigned __int64);            // number of channels2
        *input_lmf >> i64temp;
        max_number_of_hits2 = __int32(i64temp);
        byte_counter += sizeof(unsigned __int64);            // number of hits2
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32);
        ...
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32);
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32);        // Sync Mode    (parameter 60)
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32);        // IO address 2 (parameter 61)
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32);
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32);
    ...
    if (DAQVersion >= 20110208) {
        ...
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32); // indicator 2nd module
        ...
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32); // gate delay
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32); // gate open
        *input_lmf >> iDummy;
        byte_counter += sizeof(__int32); // write empty events
        *input_lmf >> iDummy;
    ...
    *input_lmf >> number_of_channels2;
    byte_counter += sizeof(__int32);    // number of channels2
    *input_lmf >> max_number_of_hits2;
    byte_counter += sizeof(__int32);    // number of hits2
    *input_lmf >> data_format_in_userheader;
    byte_counter += sizeof(__int32);                // data format (2=short integer)
...
//	TRY
    ...
    if (TDC8PCI2.use_normal_method_2nd_card) {
        ...
        *input_lmf >> TDC8PCI2.sync_test_on_off;
        byte_counter += sizeof(__int32); // sync test on/off
        *input_lmf >> TDC8PCI2.io_address_2nd_card;
        byte_counter += sizeof(__int32); // io address 2nd card
        *input_lmf >> TDC8PCI2.GateDelay_2nd_card;
        byte_counter += sizeof(__int32); // gate delay 2nd card
        *input_lmf >> TDC8PCI2.OpenTime_2nd_card;
        byte_counter += sizeof(__int32); // open time 2nd card
        *input_lmf >> TDC8PCI2.WriteEmptyEvents_2nd_card;
        byte_counter += sizeof(__int32); // write empty events 2nd card
        *input_lmf >> TDC8PCI2.TriggerFallingEdge_2nd_card;
        byte_counter += sizeof(__int32); // trigger falling edge 2nd card
        *input_lmf >> TDC8PCI2.TriggerRisingEdge_2nd_card;
        byte_counter += sizeof(__int32); // trigger rising edge 2nd card
        *input_lmf >> TDC8PCI2.EmptyCounter_2nd_card;
        byte_counter += sizeof(__int32); // EmptyCounter 2nd card
        *input_lmf >> TDC8PCI2.EmptyCounter_since_last_Event_2nd_card;
        byte_counter += sizeof(__int32); // Empty Counter since last event 2nd card
    } else {
        *input_lmf >> module_2nd;
        byte_counter += sizeof(__int32);    // indicator for 2nd module data
        *input_lmf >> TDC8PCI2.GateDelay_1st_card;
...
/////////////////////////////////////////////////////////////////
bool LMF_IO::ReadNextfADC4packet(ndigo_packet *packet, bool &bEnd_of_group_detected, __int16 *i16buffer,
    ...
    while (fADC4.packet_count > 0) {
        ...
        *input_lmf >> packet->timestamp;
        ...
        if (packet->type == CRONO_PACKET_TYPE_16_BIT_SIGNED) {
            __int32 length = __int32(packet->length * 4) < buffersize_in_16bit_words ? packet->length * 4
                                                                                     : buffersize_in_16bit_words;
...
/////////////////////////////////////////////////////////////////
bool LMF_IO::ReadNextfADC8Signal(fADC8_signal_info_struct &signal_info, bool &bEnd_of_group_detected,
...
//L100:
    ...
    if (signal_info.signal_type != 8) {
        __int32 leng = buffersize_in_32bit_words > signal_info.signallength_including_header_in_32bit_words
                       ? signal_info.signallength_including_header_in_32bit_words : buffersize_in_32bit_words;
        ...
        if (leng < signal_info.signallength_including_header_in_32bit_words) {
            __int8 byte_dummy;
            for (__int32 i = 0; i < (signal_info.signallength_including_header_in_32bit_words - leng) * 4; i++)
                input_lmf->read(&byte_dummy, 1);
        }
    }
...
__int32 LMF_IO::ReadfADC8_header_LMFversion9() {
    ...
    *input_lmf >> this->number_of_DAQ_source_strings;
    byte_counter += sizeof(__int32);
    ...
    *input_lmf >> data_format_in_userheader;
    ...
    // bools
    __int32 counter = 0;
    *input_lmf >> fADC8.number_of_bools;
    byte_counter += sizeof(__int32);
    for (__int32 i = counter; i < fADC8.number_of_bools; ++i) {
        bool bool_dummy;
        *input_lmf >> bool_dummy;
        byte_counter += sizeof(bool);
    }


    // unsigned 32bit
    counter = 0;
    *input_lmf >> fADC8.number_of_uint32s;
    byte_counter += sizeof(__int32);

    *input_lmf >> fADC8.i32NumberOfDAQLoops;
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.i32NumberOfADCmodules;
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iEnableGroupMode;
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iTriggerChannel;
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iPreSamplings_in_4800ps_units;
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iPostSamplings_in_9600ps_units;
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iEnableTDCinputs;
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iChannelMode[0][0];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iChannelMode[0][1];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iChannelMode[1][0];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iChannelMode[1][1];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iChannelMode[2][0];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iChannelMode[2][1];
    byte_counter += sizeof(__int32);
    counter++;
    for (__int32 i = counter; i < fADC8.number_of_uint32s; ++i) {
        unsigned __int32 uint_dummy;
        *input_lmf >> uint_dummy;
        byte_counter += sizeof(unsigned __int32);
    }


    // signed 32bit
    counter = 0;
    *input_lmf >> fADC8.number_of_int32s;
    byte_counter += sizeof(__int32);

    for (__int32 imod = 0; imod < 3; imod++) {
        for (__int32 ich = 0; ich < 8; ich++) {
            *input_lmf >> fADC8.iThreshold_GT[imod][ich];
            byte_counter += sizeof(__int32);
            counter++;
        }
    }
    for (__int32 imod = 0; imod < 3; imod++) {
        for (__int32 ich = 0; ich < 8; ich++) {
            *input_lmf >> fADC8.iThreshold_LT[imod][ich];
            byte_counter += sizeof(__int32);
            counter++;
        }
    }
    *input_lmf >> fADC8.iSynchronMode[0][0];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iSynchronMode[0][1];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iSynchronMode[1][0];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iSynchronMode[1][1];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iSynchronMode[2][0];
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.iSynchronMode[2][1];
    byte_counter += sizeof(__int32);
    counter++;
    ...
    // doubles
    counter = 0;
    *input_lmf >> fADC8.number_of_doubles;
    byte_counter += sizeof(__int32);
    *input_lmf >> fADC8.dGroupRangeStart;
    byte_counter += sizeof(double);
    counter++;
    *input_lmf >> fADC8.dGroupRangeEnd;
    byte_counter += sizeof(double);
    counter++;
    *input_lmf >> fADC8.dSyncTimeOffset[0][0];
    byte_counter += sizeof(double);
    counter++;
    *input_lmf >> fADC8.dSyncTimeOffset[0][1];
    byte_counter += sizeof(double);
    counter++;
    *input_lmf >> fADC8.dSyncTimeOffset[1][0];
    byte_counter += sizeof(double);
    counter++;
    *input_lmf >> fADC8.dSyncTimeOffset[1][1];
    byte_counter += sizeof(double);
    counter++;
    *input_lmf >> fADC8.dSyncTimeOffset[2][0];
    byte_counter += sizeof(double);
    counter++;
    *input_lmf >> fADC8.dSyncTimeOffset[2][1];
    byte_counter += sizeof(double);
    counter++;
    ...
    *input_lmf >> fADC8.GroupEndMarker;
    byte_counter += sizeof(__int32);
    counter++;
...
__int32 LMF_IO::ReadfADC8_header_LMFversion10() {
    ...
    *input_lmf >> fADC8.iChannelMode[2][1];
    ...
    for (__int32 i = 0; i < 3; i++) {
        *input_lmf >> fADC8.firmware_version[i];;
        byte_counter += sizeof(__int32);
        counter++; // adc module firmware
        *input_lmf >> fADC8.serial_number[i];;
        byte_counter += sizeof(__int32);
        counter++; // adc module serial number
    }

    *input_lmf >> fADC8.driver_version;
    byte_counter += sizeof(__int32);
    counter++; // adc driver version number
    ...
    *input_lmf >> fADC8.veto_gate_length;
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.veto_delay_length;
    byte_counter += sizeof(__int32);
    counter++;
    *input_lmf >> fADC8.veto_mask;
    byte_counter += sizeof(__int32);
    counter++;

    for (__int32 imod = 0; imod < 3; imod++) {
        for (__int32 ich = 0; ich < 8; ich++) {
            *input_lmf >> fADC8.GND_level[imod][ich];
            byte_counter += sizeof(__int32);
            counter++;
        }
    }
...
__int32 LMF_IO::WritefADC8_header_LMFversion10() {
    ...
    *output_lmf << LMF_Version_output;
    ...
    if ((number_of_DAQ_source_strings_output < 0) || (!DAQ_source_strings_output))
        number_of_DAQ_source_strings_output = 0;
    *output_lmf << number_of_DAQ_source_strings_output;
    byte_counter += sizeof(__int32);
    ...
    *output_lmf << data_format_in_userheader_output;
    ...
    // bools
    __int32 counter = 0;
    *output_lmf << fADC8.number_of_bools;
    byte_counter += sizeof(__int32);
    for (__int32 i = counter; i < fADC8.number_of_bools; ++i) {
        bool bool_dummy = false;
        *output_lmf << bool_dummy;
        byte_counter += sizeof(bool);
    }

    // unsigned 32bit
    counter = 0;
    *output_lmf << fADC8.number_of_uint32s;
    byte_counter += sizeof(__int32);

    *output_lmf << fADC8.i32NumberOfDAQLoops;
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.i32NumberOfADCmodules;
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iEnableGroupMode;
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iTriggerChannel;
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iPreSamplings_in_4800ps_units;
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iPostSamplings_in_9600ps_units;
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iEnableTDCinputs;
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iChannelMode[0][0];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iChannelMode[0][1];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iChannelMode[1][0];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iChannelMode[1][1];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iChannelMode[2][0];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iChannelMode[2][1];
    byte_counter += sizeof(__int32);
    counter++;

    for (__int32 i = 0; i < 3; i++) {
        *output_lmf << fADC8.firmware_version[i];
        byte_counter += sizeof(__int32);
        counter++; // adc module firmware
        *output_lmf << fADC8.serial_number[i];
        byte_counter += sizeof(__int32);
        counter++; // adc module serial number
    }

    *output_lmf << fADC8.driver_version;
    byte_counter += sizeof(__int32);
    counter++; // adc driver version number

    for (__int32 i = counter; i < fADC8.number_of_uint32s; ++i) {
        unsigned __int32 uint_dummy = 0;
        *output_lmf << uint_dummy;
        byte_counter += sizeof(unsigned __int32);
    }

    // signed 32bit
    counter = 0;
    *output_lmf << fADC8.number_of_int32s;
    byte_counter += sizeof(__int32);

    for (__int32 imod = 0; imod < 3; imod++) {
        for (__int32 ich = 0; ich < 8; ich++) {
            *output_lmf << fADC8.iThreshold_GT[imod][ich];
            byte_counter += sizeof(__int32);
            counter++;
        }
    }
    for (__int32 imod = 0; imod < 3; imod++) {
        for (__int32 ich = 0; ich < 8; ich++) {
            *output_lmf << fADC8.iThreshold_LT[imod][ich];
            byte_counter += sizeof(__int32);
            counter++;
        }
    }
    *output_lmf << fADC8.iSynchronMode[0][0];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iSynchronMode[0][1];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iSynchronMode[1][0];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iSynchronMode[1][1];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iSynchronMode[2][0];
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.iSynchronMode[2][1];
    byte_counter += sizeof(__int32);
    counter++;

    *output_lmf << fADC8.GroupEndMarker;
    byte_counter += sizeof(__int32);
    counter++;

    *output_lmf << fADC8.veto_gate_length;
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.veto_delay_length;
    byte_counter += sizeof(__int32);
    counter++;
    *output_lmf << fADC8.veto_mask;
    byte_counter += sizeof(__int32);
    counter++;

    for (__int32 imod = 0; imod < 3; imod++) {
        for (__int32 ich = 0; ich < 8; ich++) {
            *output_lmf << fADC8.GND_level[imod][ich];
            byte_counter += sizeof(__int32);
            counter++;
        }
    }
    ...
    if (counter < fADC8.number_of_int32s) {
        for (__int32 i = counter; i < fADC8.number_of_int32s; ++i) {
            *output_lmf << __int32(fADC8.bReadCustomData ? 1 : 0);
            byte_counter += sizeof(__int32);
        }
    }
    ...
    // doubles
    counter = 0;
    *output_lmf << fADC8.number_of_doubles;
    byte_counter += sizeof(__int32);
    *output_lmf << fADC8.dGroupRangeStart;
    byte_counter += sizeof(double);
    counter++;
    *output_lmf << fADC8.dGroupRangeEnd;
    byte_counter += sizeof(double);
    counter++;
    *output_lmf << fADC8.dSyncTimeOffset[0][0];
    byte_counter += sizeof(double);
    counter++;
    *output_lmf << fADC8.dSyncTimeOffset[0][1];
    byte_counter += sizeof(double);
    counter++;
    *output_lmf << fADC8.dSyncTimeOffset[1][0];
    byte_counter += sizeof(double);
    counter++;
    *output_lmf << fADC8.dSyncTimeOffset[1][1];
    byte_counter += sizeof(double);
    counter++;
    *output_lmf << fADC8.dSyncTimeOffset[2][0];
    byte_counter += sizeof(double);
    counter++;
    *output_lmf << fADC8.dSyncTimeOffset[2][1];
    byte_counter += sizeof(double);
    counter++;
    for (__int32 i = counter; i < fADC8.number_of_doubles; ++i) {
        double double_dummy = 0.;
        *output_lmf << double_dummy;
        byte_counter += sizeof(double);
    }
...
/////////////////////////////////////////////////////////////////
{
    ...
    *input_lmf >> DAQ_info_Length;
    byte_counter += sizeof(__int32);    // Length of DAQInfo always 7th value
    __int8 *__int8_temp = new __int8[DAQ_info_Length + 1];
    input_lmf->read(__int8_temp, DAQ_info_Length);
    byte_counter += DAQ_info_Length;  // DAQInfo always 8th value
    __int8_temp[DAQ_info_Length] = 0;
    DAQ_info = __int8_temp;
    delete[] __int8_temp;
    __int8_temp = 0;
...
__int32 LMF_IO::ReadfADC4Header_up_to_v11() {
    ...
    *input_lmf >> fADC4.i32NumberOfADCmodules;
    byte_counter += sizeof(__int32);


    __int32 check_value;
    *input_lmf >> check_value;
    byte_counter += sizeof(__int32);

    if (check_value == 12345678) {
        for (__int32 i = 0; i < fADC4.i32NumberOfADCmodules; i++) {
            *input_lmf >> fADC4.ndigo_parameters[i].bandwidth;
            byte_counter += sizeof(double);
            *input_lmf >> fADC4.ndigo_parameters[i].board_id;
            byte_counter += sizeof(__int32);
            *input_lmf >> fADC4.ndigo_parameters[i].channel_mask;
            byte_counter += sizeof(__int32);
            *input_lmf >> fADC4.ndigo_parameters[i].channels;
            byte_counter += sizeof(__int32);
            *input_lmf >> fADC4.ndigo_parameters[i].sample_period;
            byte_counter += sizeof(double);
            *input_lmf >> fADC4.ndigo_parameters[i].sample_rate;
            byte_counter += sizeof(double);
        ...
        byte_counter -= sizeof(__int32);
        for (__int32 i = 0; i < fADC4.i32NumberOfADCmodules; i++) {
            input_lmf->read((char *) &fADC4.ndigo_parameters[i], sizeof(ndigo_param_info));
            byte_counter += sizeof(ndigo_param_info);
            __int32 size_diff = sizeof(ndigo_param_info) - fADC4.ndigo_parameters[i].size;
            if (size_diff) {
                pos = input_lmf->tell();
                input_lmf->seek(pos - size_diff);
                byte_counter -= size_diff;
            }
        }
    ...
    for (unsigned __int32 i = 0; i < number_of_channels; i++) {
        *input_lmf >> fADC4.threshold_GT[i];
        byte_counter += sizeof(double);
        *input_lmf >> fADC4.threshold_LT[i];
        byte_counter += sizeof(double);
    }

    for (unsigned __int32 i = 0; i < number_of_channels; i++) {
        *input_lmf >> fADC4.set_DC_offset[i];
        byte_counter += sizeof(double);
    }

    if (LMF_Version > 10) {
        for (unsigned __int32 i = 0; i < number_of_channels; i++) {
            *input_lmf >> fADC4.GND_level[i];
            byte_counter += sizeof(double);
        }
    ...
    for (__int32 i = 0; i < num_temp; i++) { // write ADC mode 4x1.25Gs or 2x2.5Gs or 1x5Gs)
        *input_lmf >> fADC4.sampling_mode[i];
        byte_counter += sizeof(__int32);
    }

    *input_lmf >> i32Dummy;
    byte_counter += sizeof(__int32); // precursor in steps of 3.2 ns
    *input_lmf >> i32Dummy;
    byte_counter += sizeof(__int32); // length after end of signal in steps of 3.2 ns
    for (unsigned __int32 i = 2; i < number_of_channels; i++) {
        *input_lmf >> i32Dummy;
        byte_counter += sizeof(__int32); // read precursor and length for all other channels. Currently only for future use
        *input_lmf >> i32Dummy;
        byte_counter += sizeof(__int32);
    }

    *input_lmf >> ui64Dummy;
    byte_counter += sizeof(__int64);                // ui64RisingEnable
    *input_lmf >> ui64Dummy;
    byte_counter += sizeof(__int64);                // ui64FallingEnable
    *input_lmf >> i32Dummy;
    byte_counter += sizeof(__int32);                // i32TriggerEdge
    *input_lmf >> fADC4.iTriggerChannel;
    byte_counter += sizeof(__int32);    // i32TriggerChannel
    fADC4.iTriggerChannel--;

    *input_lmf >> bDummy;
    byte_counter += sizeof(bool);                    // bOutputLevel
    *input_lmf >> fADC4.dGroupRangeStart;
    byte_counter += sizeof(double);    // dGroupRangeStart
    *input_lmf >> fADC4.dGroupRangeEnd;
    byte_counter += sizeof(double);    // dGroupRangeEnd

    *input_lmf >> i32Dummy;
    byte_counter += sizeof(__int32);                // read Config file
    fADC4.csConfigFile = "";
    for (__int32 i32Count = 0; i32Count < i32Dummy; i32Count++) {
        input_lmf->read(&cDummy, 1);
        byte_counter += sizeof(char);
        fADC4.csConfigFile += cDummy;
    }
    *input_lmf >> i32Dummy;
    byte_counter += sizeof(__int32);                // read INL file
    fADC4.csINLFile = "";
    for (__int32 i32Count = 0; i32Count < i32Dummy; i32Count++) {
        input_lmf->read(&cDummy, 1);
        byte_counter += sizeof(char);
    ...
    *input_lmf >> i32Dummy;
    byte_counter += sizeof(__int32);                // read DNL file
    fADC4.csDNLFile = "";
    for (__int32 i32Count = 0; i32Count < i32Dummy; i32Count++) {
        input_lmf->read(&cDummy, 1);
        byte_counter += sizeof(char);
    ...
    *input_lmf >> i32Dummy;
    byte_counter += sizeof(__int32);    // dummy
    *input_lmf >> dDummy;
    byte_counter += sizeof(double);        // dGroupTimeout

    // read TDCInfo
    *input_lmf >> TDC8HP.Number_of_TDCs;
    byte_counter += sizeof(__int32);
    for (__int32 iCount = 0; iCount < TDC8HP.Number_of_TDCs; ++iCount) {
        *input_lmf >> TDC8HP.TDC_info[iCount]->index;
        byte_counter += sizeof(__int32);
        *input_lmf >> TDC8HP.TDC_info[iCount]->channelCount;
        byte_counter += sizeof(__int32);
        *input_lmf >> TDC8HP.TDC_info[iCount]->channelStart;
        byte_counter += sizeof(__int32);
        *input_lmf >> TDC8HP.TDC_info[iCount]->highResChannelCount;
        byte_counter += sizeof(__int32);
        *input_lmf >> TDC8HP.TDC_info[iCount]->highResChannelStart;
        byte_counter += sizeof(__int32);
        *input_lmf >> TDC8HP.TDC_info[iCount]->lowResChannelCount;
        byte_counter += sizeof(__int32);
        *input_lmf >> TDC8HP.TDC_info[iCount]->lowResChannelStart;
        byte_counter += sizeof(__int32);
        *input_lmf >> TDC8HP.TDC_info[iCount]->resolution;
        byte_counter += sizeof(double);
        *input_lmf >> TDC8HP.TDC_info[iCount]->serialNumber;
        byte_counter += sizeof(__int32);
        *input_lmf >> TDC8HP.TDC_info[iCount]->version;
        byte_counter += sizeof(__int32);
        *input_lmf >> TDC8HP.TDC_info[iCount]->fifoSize;
        byte_counter += sizeof(__int32);
        input_lmf->read((__int8 *) TDC8HP.TDC_info[iCount]->INLCorrection, sizeof(__int32) * 8 * 1024);
        byte_counter += sizeof(__int32) * 8 * 1024;
        input_lmf->read((__int8 *) TDC8HP.TDC_info[iCount]->DNLData, sizeof(unsigned __int16) * 8 * 1024);
        byte_counter += sizeof(unsigned __int16) * 8 * 1024;
        *input_lmf >> TDC8HP.TDC_info[iCount]->flashValid;
        byte_counter += sizeof(bool);
    }

    // now handle additional bools
    *input_lmf >> fADC4.number_of_bools;
    byte_counter += sizeof(__int32);
    ...
    *input_lmf >> fADC4.number_of_int32s;
    byte_counter += sizeof(__int32);
    if (fADC4.number_of_int32s == 3 && this->LMF_Version == 11)
        this->LMF_Version = 12; // dirty work around for old LMFs

    __int32 counter = fADC4.number_of_int32s;
    if (counter > 0) {
        *input_lmf >> fADC4.i32NumberOfDAQLoops;
        counter--;
        byte_counter += sizeof(__int32);
    }
    if (counter > 0) {
        *input_lmf >> TDC8HP.TDC8HP_DriverVersion;
        counter--;
        byte_counter += sizeof(__int32);
    }
    while (counter > 0) {
    ...
    *input_lmf >> fADC4.number_of_doubles;
    byte_counter += sizeof(__int32);
    counter = fADC4.number_of_doubles;
    if (LMF_Version > 10) {
        for (__int32 i = 0; i < fADC4.i32NumberOfADCmodules; i++) {
            *input_lmf >> fADC4.bits_per_mVolt[i];
            byte_counter += sizeof(double);
            counter--;
        }
    } else {
...
//		in_ar = new CArchive(input_lmf,CArchive::load);
    ...
    *input_lmf >> data_format_in_userheader;
    byte_counter += sizeof(__int32);    // data format (2=short integer)
    *input_lmf >> TDC8HP.no_config_file_read;
    byte_counter += sizeof(__int32);    // parameter 60-1
    if (DAQVersion < 20080000 || bRead32bit) {
        unsigned __int32 temp;
        *input_lmf >> temp;
        TDC8HP.RisingEnable_p61 = temp;
        byte_counter += sizeof(__int32);    // parameter 61-1
        *input_lmf >> temp;
        TDC8HP.FallingEnable_p62 = temp;
        byte_counter += sizeof(__int32);    // parameter 62-1
    } else {
        *input_lmf >> TDC8HP.RisingEnable_p61;
        byte_counter += sizeof(__int64);    // parameter 61-1
        *input_lmf >> TDC8HP.FallingEnable_p62;
        byte_counter += sizeof(__int64);    // parameter 62-1
    }
    *input_lmf >> TDC8HP.TriggerEdge_p63;
    byte_counter += sizeof(__int32);    // parameter 63-1
    *input_lmf >> TDC8HP.TriggerChannel_p64;
    byte_counter += sizeof(__int32);    // parameter 64-1
    *input_lmf >> TDC8HP.OutputLevel_p65;
    byte_counter += sizeof(__int32);    // parameter 65-1
    *input_lmf >> TDC8HP.GroupingEnable_p66;
    byte_counter += sizeof(__int32);    // parameter 66-1
    *input_lmf >> TDC8HP.AllowOverlap_p67;
    byte_counter += sizeof(__int32);    // parameter 67-1
    *input_lmf >> TDC8HP.TriggerDeadTime_p68;
    byte_counter += sizeof(double);        // parameter 68-1
    *input_lmf >> TDC8HP.GroupRangeStart_p69;
    byte_counter += sizeof(double);        // parameter 69-1
    *input_lmf >> TDC8HP.GroupRangeEnd_p70;
    byte_counter += sizeof(double);        // parameter 70-1
    *input_lmf >> TDC8HP.ExternalClock_p71;
    byte_counter += sizeof(__int32);    // parameter 71-1
    *input_lmf >> TDC8HP.OutputRollOvers_p72;
    byte_counter += sizeof(__int32);    // parameter 72-1
    *input_lmf >> TDC8HP.DelayTap0_p73;
    byte_counter += sizeof(__int32);    // parameter 73-1
    *input_lmf >> TDC8HP.DelayTap1_p74;
    byte_counter += sizeof(__int32);    // parameter 74-1
    *input_lmf >> TDC8HP.DelayTap2_p75;
    byte_counter += sizeof(__int32);    // parameter 75-1
    *input_lmf >> TDC8HP.DelayTap3_p76;
    byte_counter += sizeof(__int32);    // parameter 76-1
    *input_lmf >> TDC8HP.INL_p80;
    byte_counter += sizeof(__int32);    // parameter 80-1
    *input_lmf >> TDC8HP.DNL_p81;
    byte_counter += sizeof(__int32);    // parameter 81-1
    ...
    *input_lmf >> TDC8HP.csDNLFile_Length;
    ...
    if (DAQVersion < 20080000) {
        ...
        *input_lmf >> TDC8HP.GroupTimeOut;
        byte_counter += sizeof(double);

        *input_lmf >> TDC8HP.SSEEnable;
        byte_counter += sizeof(bool);
        *input_lmf >> TDC8HP.MMXEnable;
        byte_counter += sizeof(bool);
        *input_lmf >> TDC8HP.DMAEnable;
        byte_counter += sizeof(bool);
        ...
        if (!bNoTDCInfoRead) {
            ...
            *input_lmf >> TDC8HP.Number_of_TDCs;
            byte_counter += sizeof(__int32);
            if (TDC8HP.Number_of_TDCs < 0 || TDC8HP.Number_of_TDCs > 3) {
                bNoTDCInfoRead = true;
                goto L200;
            }
            for (__int32 iCount = 0; iCount < TDC8HP.Number_of_TDCs; ++iCount) {
        ...
        *input_lmf >> TDC8HP.i32NumberOfDAQLoops;
        byte_counter += sizeof(__int32);
        ...
        *input_lmf >> TDC8HP.iTime_zero_channel;
        ...
        if (TDC8HP.iTime_zero_channel != 0x12345678) {
            if (TDC8HP.iTime_zero_channel < -1 || TDC8HP.iTime_zero_channel > 50) {
                bNoTDCInfoRead = true;
                goto L200;
            }
        }
...
//	input_lmf->flush();
    ...
    *input_lmf >> temp_bool;
    TDC8HP.no_config_file_read = temp_bool;
    byte_counter += sizeof(bool);    // parameter 60-1
    ...
    *input_lmf >> temp_bool;
    TDC8HP.OutputLevel_p65 = temp_bool;
    byte_counter += sizeof(bool);    // parameter 65-1
    *input_lmf >> temp_bool;
    TDC8HP.GroupingEnable_p66 = temp_bool;
    byte_counter += sizeof(bool);    // parameter 66-1
    *input_lmf >> temp_bool;
    TDC8HP.AllowOverlap_p67 = temp_bool;
    byte_counter += sizeof(bool);    // parameter 67-1
    ...
    *input_lmf >> temp_bool;
    TDC8HP.ExternalClock_p71 = temp_bool;
    byte_counter += sizeof(bool);    // parameter 71-1
    *input_lmf >> temp_bool;
    TDC8HP.OutputRollOvers_p72 = temp_bool;
    byte_counter += sizeof(bool);    // parameter 72-1
    ...
    *input_lmf >> temp_bool;
    TDC8HP.INL_p80 = temp_bool;
    byte_counter += sizeof(bool);    // parameter 80-1
    *input_lmf >> temp_bool;
    TDC8HP.DNL_p81 = temp_bool;
    byte_counter += sizeof(bool);    // parameter 81-1
    ...
    *input_lmf >> TDC8HP.number_of_bools;
    byte_counter += sizeof(__int32);
    ...
    *input_lmf >> TDC8HP.number_of_int32s;
    byte_counter += sizeof(__int32);
    ...
    *input_lmf >> TDC8HP.iTriggerChannelMask;
    byte_counter += sizeof(__int32);
    *input_lmf >> TDC8HP.iTime_zero_channel;
    byte_counter += sizeof(__int32); // 1 is first channel

    if (TDC8HP.UserHeaderVersion >= 6) {
        *input_lmf >> TDC8HP.BinsizeType;
        byte_counter += sizeof(__int32); // 1 is first channel
    ...
    *input_lmf >> TDC8HP.number_of_doubles;
    byte_counter += sizeof(__int32);
    if (TDC8HP.UserHeaderVersion == 5) {
    ...
    if (TDC8HP.UserHeaderVersion >= 6) {
        *input_lmf >> TDC8HP.OffsetTimeZeroChannel_s;
        byte_counter += sizeof(double);
...
//	input_lmf->flush();
    ...
    *input_lmf >> temp_uint64;
    ...
    if (__int32(number_of_channels) > num_channels) {
        errorflag = 19;
        return -100000;
    }
    if (__int32(max_number_of_hits) > num_ions) {
        errorflag = 20;
        return -100000;
    }
    ...
    *input_lmf >> TDC8HP.iTriggerChannelMask;
    byte_counter += sizeof(__int32);
    *input_lmf >> TDC8HP.iTime_zero_channel;
    byte_counter += sizeof(__int32); // 1 means: first channel
    *input_lmf >> TDC8HP.BinsizeType;
    byte_counter += sizeof(__int32); // 1 means: first channel
    for (__int32 i = 5; i < TDC8HP.number_of_int32s; ++i) {
        __int32 int_dummy;
...
/////////////////////////////////////////////////////////////////
{
    ...
    *input_lmf >> time_reference;
    byte_counter += sizeof(__int32);
    *input_lmf >> HM1.FAK_DLL_Value;
    byte_counter += sizeof(__int32);
    *input_lmf >> HM1.Resolution_Flag;
    byte_counter += sizeof(__int32);
    *input_lmf >> HM1.trigger_mode_for_start;
    byte_counter += sizeof(__int32);
    *input_lmf >> HM1.trigger_mode_for_stop;
    byte_counter += sizeof(__int32);
    *input_lmf >> tdcresolution;
    byte_counter += sizeof(double);        // tdc resolution in ns
    ...
    *input_lmf >> HM1.Even_open_time;
    byte_counter += sizeof(__int32);
    *input_lmf >> HM1.Auto_Trigger;
    byte_counter += sizeof(__int32);

    if (DAQVersion >= 20020408) {
        __int64 i64_dummy;
        *input_lmf >> i64_dummy;
        number_of_channels = (unsigned __int32) i64_dummy;
        byte_counter += sizeof(__int64);            // number of channels
        *input_lmf >> i64_dummy;
        max_number_of_hits = (unsigned __int32) i64_dummy;
        byte_counter += sizeof(__int64);            // number of hits
    } else {
        *input_lmf >> number_of_channels;
        byte_counter += sizeof(__int32);            // number of channels
    ...
    *input_lmf >> HM1.set_bits_for_GP1;
    byte_counter += sizeof(__int32);
    *input_lmf >> data_format_in_userheader;
    byte_counter += sizeof(__int32);                // data format (2=short integer)
    *input_lmf >> module_2nd;
    ...
    if (DAQ_ID == DAQ_ID_2HM1) {
        *input_lmf >> DAQSubVersion;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_FAK_DLL_Value;
        byte_counter += sizeof(__int32);  // parameter 10-1
        *input_lmf >> HM1.TWOHM1_Resolution_Flag;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_trigger_mode_for_start;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_trigger_mode_for_stop;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_res_adjust;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_tdcresolution;
        byte_counter += sizeof(double);
        *input_lmf >> HM1.TWOHM1_test_overflow;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_number_of_channels;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_number_of_hits;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_set_bits_for_GP1;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_HM1_ID_1;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.TWOHM1_HM1_ID_2;
        byte_counter += sizeof(__int32);
    }

    if (DAQ_ID == DAQ_ID_HM1_ABM) {
        max_number_of_hits = 1;
        *input_lmf >> HM1.ABM_m_xFrom;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_xTo;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_yFrom;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_yTo;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_xMin;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_xMax;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_yMin;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_yMax;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_xOffset;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_yOffset;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_m_zOffset;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_Mode;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_OsziDarkInvert;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_ErrorHisto;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_XShift;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_YShift;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_ZShift;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_ozShift;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_wdShift;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_ucLevelXY;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_ucLevelZ;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_uiABMXShift;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_uiABMYShift;
        byte_counter += sizeof(__int32);
        *input_lmf >> HM1.ABM_uiABMZShift;
        byte_counter += sizeof(__int32);
        if (DAQVersion > 20080507) {
            __int32 i32Dummy;
            *input_lmf >> i32Dummy;
...
/////////////////////////////////////////////////////////////////
{
    ...
    *output_lmf << max_number_of_hits_output;
    byte_counter += sizeof(__int32);            // number of hits
    *output_lmf << number_of_channels2_output;
    byte_counter += sizeof(__int32);    // number of channels2
    *output_lmf << max_number_of_hits2_output;
    byte_counter += sizeof(__int32);    // number of hits2
    *output_lmf << data_format_in_userheader_output;
    byte_counter += sizeof(__int32);                // data format (2=short integer)
    ...
    if (TDC8PCI2.use_normal_method_2nd_card) {
        ...
        *output_lmf << TDC8PCI2.sync_test_on_off;
        byte_counter += sizeof(__int32); // sync test on/off
        *output_lmf << TDC8PCI2.io_address_2nd_card;
        byte_counter += sizeof(__int32); // io address 2nd card
        *output_lmf << TDC8PCI2.GateDelay_2nd_card;
        byte_counter += sizeof(__int32); // gate delay 2nd card
        *output_lmf << TDC8PCI2.OpenTime_2nd_card;
        byte_counter += sizeof(__int32); // open time 2nd card
        *output_lmf << TDC8PCI2.WriteEmptyEvents_2nd_card;
        byte_counter += sizeof(__int32); // write empty events 2nd card
        *output_lmf << TDC8PCI2.TriggerFallingEdge_2nd_card;
        byte_counter += sizeof(__int32); // trigger falling edge 2nd card
        *output_lmf << TDC8PCI2.TriggerRisingEdge_2nd_card;
        byte_counter += sizeof(__int32); // trigger rising edge 2nd card
        *output_lmf << TDC8PCI2.EmptyCounter_2nd_card;
        byte_counter += sizeof(__int32); // EmptyCounter 2nd card
        *output_lmf << TDC8PCI2.EmptyCounter_since_last_Event_2nd_card;
        byte_counter += sizeof(__int32); // Empty Counter since last event 2nd card
    } else {
        *output_lmf << module_2nd;
        byte_counter += sizeof(__int32); // indicator for 2nd module data
...
/////////////////////////////////////////////////////////////////
{
    ...
    *output_lmf << tdcresolution_output;
    ...
    if (DAQVersion_output >= 20080000) {
        unsigned __int64 temp;
        temp = number_of_channels_output;
        *output_lmf << temp;
        byte_counter += sizeof(unsigned __int64);            // number of channels
        temp = max_number_of_hits_output;
        *output_lmf << temp;
        byte_counter += sizeof(unsigned __int64);            // number of hits
    }
    *output_lmf << data_format_in_userheader_output;
    ...
    *output_lmf << TDC8HP.no_config_file_read;
    byte_counter += sizeof(__int32);    // parameter 60-1
    if (DAQVersion_output < 20080000) {
        unsigned __int32 temp;
        temp = __int32(TDC8HP.RisingEnable_p61);
        *output_lmf << temp;
        byte_counter += sizeof(__int32);    // parameter 61-1
        temp = __int32(TDC8HP.FallingEnable_p62);
        *output_lmf << temp;
        byte_counter += sizeof(__int32);    // parameter 62-1
    }
    if (DAQVersion_output >= 20080000) {
        *output_lmf << TDC8HP.RisingEnable_p61;
        byte_counter += sizeof(__int64);    // parameter 61-1
        *output_lmf << TDC8HP.FallingEnable_p62;
        byte_counter += sizeof(__int64);    // parameter 62-1
    }
    *output_lmf << TDC8HP.TriggerEdge_p63;
    byte_counter += sizeof(__int32);    // parameter 63-1
    *output_lmf << TDC8HP.TriggerChannel_p64;
    byte_counter += sizeof(__int32);    // parameter 64-1
    *output_lmf << TDC8HP.OutputLevel_p65;
    byte_counter += sizeof(__int32);    // parameter 65-1
    *output_lmf << TDC8HP.GroupingEnable_p66_output;
    byte_counter += sizeof(__int32);    // parameter 66-1
    *output_lmf << TDC8HP.AllowOverlap_p67;
    byte_counter += sizeof(__int32);    // parameter 67-1
    *output_lmf << TDC8HP.TriggerDeadTime_p68;
    byte_counter += sizeof(double);    // parameter 68-1
    *output_lmf << TDC8HP.GroupRangeStart_p69;
    byte_counter += sizeof(double);    // parameter 69-1
    *output_lmf << TDC8HP.GroupRangeEnd_p70;
    byte_counter += sizeof(double);    // parameter 70-1
    *output_lmf << TDC8HP.ExternalClock_p71;
    byte_counter += sizeof(__int32);    // parameter 71-1
    *output_lmf << TDC8HP.OutputRollOvers_p72;
    byte_counter += sizeof(__int32);    // parameter 72-1
    *output_lmf << TDC8HP.DelayTap0_p73;
    byte_counter += sizeof(__int32);    // parameter 73-1
    *output_lmf << TDC8HP.DelayTap1_p74;
    byte_counter += sizeof(__int32);    // parameter 74-1
    *output_lmf << TDC8HP.DelayTap2_p75;
    byte_counter += sizeof(__int32);    // parameter 75-1
    *output_lmf << TDC8HP.DelayTap3_p76;
    byte_counter += sizeof(__int32);    // parameter 76-1
    *output_lmf << TDC8HP.INL_p80;
    byte_counter += sizeof(__int32);    // parameter 80-1
    *output_lmf << TDC8HP.DNL_p81;
    byte_counter += sizeof(__int32);    // parameter 81-1
    ...
    *output_lmf << dummy;
    ...
    if (TDC8HP.UserHeaderVersion >= 2) {
        *output_lmf << TDC8HP.SyncValidationChannel;
        byte_counter += sizeof(__int32);
    }
    if (TDC8HP.UserHeaderVersion >= 3) {
        *output_lmf << TDC8HP.VHR_25ps;
        byte_counter += sizeof(bool);
    }

    if (TDC8HP.UserHeaderVersion >= 4) {

        *output_lmf << TDC8HP.GroupTimeOut;
        byte_counter += sizeof(double);

        *output_lmf << TDC8HP.SSEEnable;
        byte_counter += sizeof(bool);
        *output_lmf << TDC8HP.MMXEnable;
        byte_counter += sizeof(bool);
        *output_lmf << TDC8HP.DMAEnable;
        byte_counter += sizeof(bool);

        //// write TDCInfo
        *output_lmf << TDC8HP.Number_of_TDCs;
        byte_counter += sizeof(__int32);
        for (__int32 iCount = 0; iCount < TDC8HP.Number_of_TDCs; ++iCount) {
            *output_lmf << TDC8HP.TDC_info[iCount]->index;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.TDC_info[iCount]->channelCount;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.TDC_info[iCount]->channelStart;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.TDC_info[iCount]->highResChannelCount;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.TDC_info[iCount]->highResChannelStart;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.TDC_info[iCount]->lowResChannelCount;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.TDC_info[iCount]->lowResChannelStart;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.TDC_info[iCount]->resolution;
            byte_counter += sizeof(double);
            *output_lmf << TDC8HP.TDC_info[iCount]->serialNumber;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.TDC_info[iCount]->version;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.TDC_info[iCount]->fifoSize;
            byte_counter += sizeof(__int32);
            output_lmf->write((__int8 *) TDC8HP.TDC_info[iCount]->INLCorrection, sizeof(__int32) * 8 * 1024);
            byte_counter += sizeof(__int32) * 8 * 1024;
            output_lmf->write((__int8 *) TDC8HP.TDC_info[iCount]->DNLData, sizeof(unsigned __int16) * 8 * 1024);
            byte_counter += sizeof(unsigned __int16) * 8 * 1024;
            *output_lmf << TDC8HP.TDC_info[iCount]->flashValid;
            byte_counter += sizeof(bool);
        }
        ...
        *output_lmf << TDC8HP.i32NumberOfDAQLoops;
        byte_counter += sizeof(__int32);
        *output_lmf << TDC8HP.TDC8HP_DriverVersion;
        byte_counter += sizeof(__int32);
        *output_lmf << TDC8HP.iTriggerChannelMask;
        byte_counter += sizeof(__int32);
        *output_lmf << TDC8HP.iTime_zero_channel;
        byte_counter += sizeof(__int32);
        *output_lmf << int_Dummy;
        byte_counter += sizeof(__int32);
...
/////////////////////////////////////////////////////////////////
{
    ...
    *output_lmf << TDC8HP.number_of_bools;
    byte_counter += sizeof(__int32);
    ...
    *output_lmf << TDC8HP.number_of_int32s;
    byte_counter += sizeof(__int32);
    *output_lmf << TDC8HP.i32NumberOfDAQLoops;
    byte_counter += sizeof(__int32);
    *output_lmf << TDC8HP.TDC8HP_DriverVersion;
    ...
    *output_lmf << TDC8HP.iTime_zero_channel;
    ...
    if (TDC8HP.UserHeaderVersion == 5) {
        *output_lmf << TDC8HP.number_of_doubles;
        byte_counter += sizeof(__int32);
        for (__int32 i = 0; i < TDC8HP.number_of_doubles; ++i) {
            double ddummy = 0.;
            *output_lmf << ddummy;
    ...
    if (TDC8HP.UserHeaderVersion >= 6) {
        if (TDC8HP.number_of_doubles == 0) {
            ...
            *output_lmf << TDC8HP.number_of_doubles;
            byte_counter += sizeof(__int32);
            *output_lmf << TDC8HP.OffsetTimeZeroChannel_s;
            byte_counter += sizeof(double);
            for (__int32 i = 1; i < TDC8HP.number_of_doubles; ++i) {
                double ddummy = 0.;
                *output_lmf << ddummy;
...
/////////////////////////////////////////////////////////////////
{
    ...
    *output_lmf << dummy;
    ...
    if (DAQVersion_output >= 20020408 && HM1.use_normal_method) {
        *output_lmf << LMF_Version_output;
        byte_counter += sizeof(__int32);
    }
    ...
    *output_lmf << time_reference_output;
    ...
    if (DAQ_ID_output == DAQ_ID_HM1 || DAQ_ID_output == DAQ_ID_HM1_ABM) {
        *output_lmf << HM1.FAK_DLL_Value;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.Resolution_Flag;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.trigger_mode_for_start;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.trigger_mode_for_stop;
        byte_counter += sizeof(__int32);
    }
    *output_lmf << tdcresolution_output;
    byte_counter += sizeof(double);        // tdc resolution in ns
    ...
    if (DAQ_ID_output == DAQ_ID_HM1 || DAQ_ID_output == DAQ_ID_HM1_ABM) {
        *output_lmf << HM1.Even_open_time;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.Auto_Trigger;
        byte_counter += sizeof(__int32);
    }
    ...
    if (DAQ_ID_output == DAQ_ID_HM1 || DAQ_ID_output == DAQ_ID_HM1_ABM) {
        *output_lmf << HM1.set_bits_for_GP1;
        byte_counter += sizeof(__int32);
    }
    *output_lmf << data_format_in_userheader_output;
    byte_counter += sizeof(__int32);                // data format (2=short integer)
    ...
    if (DAQ_ID_output == DAQ_ID_HM1 || DAQ_ID_output == DAQ_ID_HM1_ABM) {
        *output_lmf << module_2nd;
        byte_counter += sizeof(__int32);
    }    // indicator for 2nd module data

    if (DAQ_ID_output == DAQ_ID_2HM1) {
        *output_lmf << DAQSubVersion;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_FAK_DLL_Value;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_Resolution_Flag;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_trigger_mode_for_start;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_trigger_mode_for_stop;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_res_adjust;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_tdcresolution;
        byte_counter += sizeof(double);
        *output_lmf << HM1.TWOHM1_test_overflow;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_number_of_channels;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_number_of_hits;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_set_bits_for_GP1;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_HM1_ID_1;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.TWOHM1_HM1_ID_2;
        byte_counter += sizeof(__int32);
    }

    if (DAQ_ID_output == DAQ_ID_HM1_ABM) {
        *output_lmf << HM1.ABM_m_xFrom;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_xTo;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_yFrom;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_yTo;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_xMin;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_xMax;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_yMin;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_yMax;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_xOffset;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_yOffset;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_m_zOffset;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_Mode;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_OsziDarkInvert;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_ErrorHisto;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_XShift;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_YShift;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_ZShift;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_ozShift;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_wdShift;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_ucLevelXY;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_ucLevelZ;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_uiABMXShift;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_uiABMYShift;
        byte_counter += sizeof(__int32);
        *output_lmf << HM1.ABM_uiABMZShift;
        byte_counter += sizeof(__int32);
    }
...
//	}
    ...
    if (number_of_DAQ_source_strings_output == -1 && number_of_DAQ_source_strings > 0) {
        ...
        for (__int32 i = 0; i < number_of_DAQ_source_strings; ++i) {
            DAQ_source_strings_output[i] = new std::string();
            *DAQ_source_strings_output[i] = *DAQ_source_strings[i];
        }
    ...
    if (number_of_CCFHistory_strings_output == -1 && number_of_CCFHistory_strings > 0) {
        ...
        for (__int32 i = 0; i < number_of_CCFHistory_strings; ++i) {
            CCFHistory_strings_output[i] = new std::string();
            *CCFHistory_strings_output[i] = *CCFHistory_strings[i];
        }
    ...
    if (number_of_DAN_source_strings_output == -1 && number_of_DAN_source_strings > 0) {
        ...
        for (__int32 i = 0; i < number_of_DAN_source_strings; ++i) {
            DAN_source_strings_output[i] = new std::string();
            *DAN_source_strings_output[i] = *DAN_source_strings[i];
        }
    ...
    if (DAQ_ID_output == DAQ_ID_RAW32BIT) {
        ...
        if (number_of_channels_output == 0 || max_number_of_hits_output == 0) {
            errorflag = 14;
            return false;
        }
...
// -----------------------------------
    ...
    if (Numberofcoordinates_output == -2) {
        ...
        if (data_format_in_userheader_output == LM_DOUBLE)
            Numberofcoordinates_output = timestamp_format_output == 0 ? 0 : 1;
        if (data_format_in_userheader_output == LM_SLONG) Numberofcoordinates_output = timestamp_format_output;
        if (data_format_in_userheader_output == LM_SHORT || data_format_in_userheader_output == LM_DOUBLE ||
            data_format_in_userheader_output == LM_SLONG) {
        ...
        if (data_format_in_userheader_output == LM_USERDEF) {
            ...
            if (DAQ_ID_output == DAQ_ID_TDC8 || DAQ_ID_output == DAQ_ID_2TDC8) {
                ...
                Numberofcoordinates_output =
                        (number_of_channels_output + number_of_channels2_output) * (1 + max_number_of_hits_output);
            }
...
//  WRITE USER-HEADER
    ...
    *output_lmf << DAQVersion_output;
    output_byte_counter += sizeof(__int32);    // Version is always 2nd value
    *output_lmf << DAQ_ID_output;
    output_byte_counter += sizeof(__int32);    // DAQ_ID is always 3ed value
...
/////////////////////////////////////////////////////////////////
{
    if (Cobold_Header_version_output >= 2008) {
        ...
        *output_lmf << data_format_in_userheader_output;
        ...
        unsigned __int64 temp;
        temp = Numberofcoordinates_output;
        *output_lmf << temp;
        temp = Headersize_output;
        *output_lmf << temp;
        temp = User_header_size_output;
        *output_lmf << temp;
        ...
        if (number_of_CCFHistory_strings_output > 0) {
            *output_lmf << number_of_CCFHistory_strings_output;
            for (__int32 i = 0; i < number_of_CCFHistory_strings_output; ++i) {
                ...
                *output_lmf << unsigned_int_Dummy;
                output_lmf->write(CCFHistory_strings_output[i]->c_str(),
                                  __int32(CCFHistory_strings_output[i]->length()));
            }
        ...
        if (number_of_DAN_source_strings_output > 0) {
            *output_lmf << number_of_DAN_source_strings_output;
            for (__int32 i = 0; i < number_of_DAN_source_strings_output; ++i) {
                ...
                *output_lmf << unsigned_int_Dummy;
                output_lmf->write(DAN_source_strings_output[i]->c_str(),
                                  __int32(DAN_source_strings_output[i]->length()));
            }
        } else *output_lmf << __int32(0);
    } else {
        number_of_DAN_source_strings_output = 0;
...
/////////////////////////////////////////////////////////////////
{
    ...
    if (DAQVersion_output >= 20080000 || data_format_in_userheader_output == LM_USERDEF) {
        ...
        HeaderLength = timestamp_format_output * sizeof(__int32) +
                       2 * sizeof(__int64);    // +2 for HeaderLength itself, +2 for EventCounter (size in __int32)
...
#else
        HeaderLength =
                (HeaderLength & 0x00ffffffffffffff) | 0xff00000000000000;    // set 0xff in bits 63..56 as EventMarker
#endif
...
/////////////////////////////////////////////////////////////////
{
    ...
    if (DAQ_ID_output == DAQ_ID_2TDC8 && number_of_channels2_output >
                                         0) { // here we write only the stuff for the second TDC (because the first TDC was written above)
        for (i = number_of_channels_output; i < number_of_channels2_output + number_of_channels_output; ++i) {
            ...
            if (data_format_in_userheader_output == 10) {
                dummy_int32 = __int32(hits + (DAQ_ID == DAQ_ID_HM1 ? 1 : 0));
...
/////////////////////////////////////////////////////////////////
{
    ...
    if (DAQ_ID_output == DAQ_ID_HM1_ABM) {
        for (i = 0; i < number_of_channels_output; ++i) {
            ...
            if (data_format_in_userheader_output == 5) {
                dummy_double = double(us16TDC[i * num_ions]);
                *output_lmf << dummy_double;
            }
            if (data_format_in_userheader_output == 10) {
                dummy_int32 = __int32(us16TDC[i * num_ions]);
                *output_lmf << dummy_int32;
            }
        }
    }
    if (DAQ_ID_output != DAQ_ID_SIMPLE && DAQ_ID_output != DAQ_ID_HM1_ABM) {
        for (i = 0; i < number_of_channels_output; ++i) {
            ...
            if (data_format_in_userheader_output == 5) {
                ...
                *output_lmf << dummy_double;
                for (j = 0; j < hits; ++j) {
                    dummy_double = double(us16TDC[i * num_ions + j]);
                    *output_lmf << dummy_double;
                }
                dummy_double = 0.;
                for (j = hits; j < max_number_of_hits_output; ++j) *output_lmf << dummy_double;
        ...
        if (data_format_in_userheader_output == 10) {
            for (channel = 0; channel < (unsigned __int32) number_of_channels_output; ++channel) {
                if (cnt[channel] > 0) {
                    ...
                    *output_lmf << dummy_int32;
                    for (i = 0; i < cnt[channel]; ++i) {
                        dummy_int32 = __int32(us16TDC[channel * num_ions + i]);
                        *output_lmf << dummy_int32;
                    }
                }
            }
...
*/
    ...
    if (input_lmf->filesize <
        (unsigned __int64) (eventsize) * target_number + (unsigned __int64) (Headersize + User_header_size))
        return false;
...
///////////////////////////////////////////////////////////////////////////////////
__int32 LMF_IO::PCIGetTDC_TDC8HP_25psGroupMode(unsigned __int64 &ref_ui64TDC8HPAbsoluteTimeStamp, __int32 count,
    ...
    if (DAQ_ID != DAQ_ID_SIMPLE && !TDC8HP.variable_event_length && !TDC8PCI2.variable_event_length) {
        for (i = 0; i < number_of_channels + number_of_channels2; ++i) {
            if (DAQ_ID == DAQ_ID_HM1_ABM) {
                ...
                if (input_lmf->error) {
                    if (input_lmf->eof) this->errorflag = 18; else this->errorflag = 2;
                    return false;
                }
            }
            if (DAQ_ID != DAQ_ID_HM1_ABM) {
...
/////////////////////////////////////////////////////////////////
{
    ...
    if (timestamp_format_output >= 1) {
        if (data_format_in_userheader_output == 2) {
            dummy_uint16 = (unsigned __int16) (LARGE_Timestamp.LowPart & 0x0000ffff);
            *output_lmf << dummy_uint16;            // 32 Bit Low part, lower  16 Bit
            dummy_uint16 = (unsigned __int16) (LARGE_Timestamp.LowPart & 0xffff0000);
            *output_lmf << dummy_uint16;            // 32 Bit Low part, higher 16 Bit
        }
        if (data_format_in_userheader_output == 6) {
            dummy_uint8 = (unsigned __int8) (LARGE_Timestamp.LowPart & 0x000000ff);
            *output_lmf << dummy_uint8;            // 32 Bit Low part, lower 16 Bit, lower 8 bit
            dummy_uint8 = (unsigned __int8) ((LARGE_Timestamp.LowPart >> 8) & 0x000000ff);
            *output_lmf << dummy_uint8;    // 32 Bit Low part, lower 16 Bit, high 8 bit
            dummy_uint8 = (unsigned __int8) 0;
            *output_lmf << dummy_uint8;
            dummy_uint8 = (unsigned __int8) ((LARGE_Timestamp.LowPart >> 16) & 0x000000ff);
            *output_lmf << dummy_uint8;    // 32 Bit Low part, lower 16 Bit, high 8 bit
            dummy_uint8 = (unsigned __int8) ((LARGE_Timestamp.LowPart >> 24) & 0x000000ff);
            *output_lmf << dummy_uint8;    // 32 Bit Low part, lower 16 Bit, high 8 bit
            dummy_uint8 = (unsigned __int8) 0;
            *output_lmf << dummy_uint8;
        }
    ...
    if (timestamp_format_output == 2) {
        if (data_format_in_userheader_output == 2) {
            dummy_uint16 = (unsigned __int16) (LARGE_Timestamp.HighPart & 0x0000ffff);
            *output_lmf << dummy_uint16;            // 32 Bit High part, lower  16 Bit
            dummy_uint16 = (unsigned __int16) (LARGE_Timestamp.HighPart & 0xffff0000);
            *output_lmf << dummy_uint16;            // 32 Bit High part, higher 16 Bit
        }
        if (data_format_in_userheader_output == 6) {
            dummy_uint8 = (unsigned __int8) (LARGE_Timestamp.HighPart & 0x000000ff);
            *output_lmf << dummy_uint8;            // 32 Bit High part, lower 16 Bit, lower 8 bit
            dummy_uint8 = (unsigned __int8) ((LARGE_Timestamp.HighPart >> 8) & 0x000000ff);
            *output_lmf << dummy_uint8;    // 32 Bit High part, lower 16 Bit, high 8 bit
            dummy_uint8 = (unsigned __int8) 0;
            *output_lmf << dummy_uint8;
            dummy_uint8 = (unsigned __int8) ((LARGE_Timestamp.HighPart >> 16) & 0x000000ff);
            *output_lmf << dummy_uint8;    // 32 Bit High part, lower 16 Bit, high 8 bit
            dummy_uint8 = (unsigned __int8) ((LARGE_Timestamp.HighPart >> 24) & 0x000000ff);
            *output_lmf << dummy_uint8;    // 32 Bit High part, lower 16 Bit, high 8 bit
            dummy_uint8 = (unsigned __int8) 0;
            *output_lmf << dummy_uint8;
        }
    ...
    for (i = 0; i < Numberofcoordinates - timestamp_format_output * 2; ++i) {
        if (data_format_in_userheader_output == 2) {
            dummy_uint16 = (unsigned __int16) (data[i] & 0x0000ffff);
            *output_lmf << dummy_uint16;
        }
        if (data_format_in_userheader_output == 6) {
            dummy_uint8 = (unsigned __int8) (data[i] & 0x000000ff);
            *output_lmf << dummy_uint8;
            dummy_uint8 = (unsigned __int8) ((data[i] >> 8) & 0x000000ff);
            *output_lmf << dummy_uint8;
            dummy_uint8 = (unsigned __int8) ((data[i] >> 16) & 0x000000ff);
            *output_lmf << dummy_uint8;
        }
    }
...
/////////////////////////////////////////////////////////////////
    ...
    for (i = 0; i < num_channels; ++i)
        cnt[i] = (number_of_hits[i] < (unsigned __int32) (num_ions)) ? number_of_hits[i] : num_ions;
}
...
/////////////////////////////////////////////////////////////////
unsigned __int32 LMF_IO::GetNumberOfChannels() const
/////////////////////////////////////////////////////////////////
{
...
/////////////////////////////////////////////////////////////////
unsigned __int32 LMF_IO::GetMaxNumberOfHits() const
/////////////////////////////////////////////////////////////////
{
...
/////////////////////////////////////////////////////////////////
{
    ...
    if (this->CCFHistory_strings) {
        for (__int32 i = 0; i < number_of_CCFHistory_strings; ++i) {
            clone->CCFHistory_strings[i] = new std::string();
            *clone->CCFHistory_strings[i] = *this->CCFHistory_strings[i];
        }
    }
    if (this->DAN_source_strings) {
        for (__int32 i = 0; i < number_of_DAN_source_strings; ++i) {
            clone->DAN_source_strings[i] = new std::string();
            *clone->DAN_source_strings[i] = *this->DAN_source_strings[i];
        }
    }
    if (this->DAQ_source_strings) {
        for (__int32 i = 0; i < number_of_DAQ_source_strings; ++i) {
            clone->DAQ_source_strings[i] = new std::string();
            *clone->DAQ_source_strings[i] = *this->DAQ_source_strings[i];
        }
    }
    ...
    if (this->CCFHistory_strings_output) {
        for (__int32 i = 0; i < number_of_CCFHistory_strings_output; ++i) {
            clone->CCFHistory_strings_output[i] = new std::string();
            *clone->CCFHistory_strings_output[i] = *this->CCFHistory_strings_output[i];
        }
    }
    if (this->DAN_source_strings_output) {
        for (__int32 i = 0; i < number_of_DAN_source_strings_output; ++i) {
            clone->DAN_source_strings_output[i] = new std::string();
            *clone->DAN_source_strings_output[i] = *this->DAN_source_strings_output[i];
        }
    }
    if (this->DAQ_source_strings_output) {
        for (__int32 i = 0; i < number_of_DAQ_source_strings_output; ++i) {
            clone->DAQ_source_strings_output[i] = new std::string();
            *clone->DAQ_source_strings_output[i] = *this->DAQ_source_strings_output[i];
        }
    }
...
//clone->output_lmf				= this->output_lmf;
    ...
    for (__int32 i = 0; i < 3; ++i) {
        if (this->TDC8HP.TDC_info[i]) {
            ...
            memcpy(clone->TDC8HP.TDC_info[i]->INLCorrection, this->TDC8HP.TDC_info[i]->INLCorrection,
                   sizeof(__int32) * 8 * 1024);
...
bool
LMF_IO::decompress_asynchronous_fADC8_signal(fADC8_signal_info_struct &signal_info, unsigned __int32 source_buffer[],
    ...
    if ((signal_info.signal_type == FADC8_HEADER_EVENT_ID_TDC) ||
        (signal_info.signal_type == FADC8_HEADER_EVENT_ID_END_MARKER)) {
    ...
    if ((signal_info.signal_type == FADC8_HEADER_EVENT_ID_1_25G_ADC1)
        ...
        for (unsigned __int32 i = 4; i < limit_ + 4; i += 4) {
            ...
            for (__int32 j = 0; j < 4; j++) {
                i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                        (source_buffer[i + j] >> 20) & 0x3ff;
                i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                        (source_buffer[i + j] >> 10) & 0x3ff;
                i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                        (source_buffer[i + j]) & 0x3ff;
            }
        }
    ...
    if ((signal_info.signal_type == FADC8_HEADER_EVENT_ID_2_5G_ADC12)
        ...
        for (unsigned __int32 i = 4; i < limit_ + 4; i += 8) {
            ...
            for (__int32 j = 0; j < 4; j++) {
                i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                        (source_buffer[i + 0 + j] >> 20) & 0x3ff;
                i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                        (source_buffer[i + 4 + j] >> 20) & 0x3ff;
                i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                        (source_buffer[i + 0 + j] >> 10) & 0x3ff;
                i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                        (source_buffer[i + 4 + j] >> 10) & 0x3ff;
                i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                        (source_buffer[i + 0 + j]) & 0x3ff;
                i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                        (source_buffer[i + 4 + j]) & 0x3ff;
            }
        ...
        for (unsigned __int32 i = 4; i < limit_ + 4; i += 16) {
            ...
            for (__int32 j = 0; j < 4; j++) {
                for (__int32 shift_ = 20; shift_ >= 0; shift_ -= 10) {
                    i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 0 + j] >> shift_) & 0x3ff;
                    i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 8 + j] >> shift_) & 0x3ff;
                    i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 4 + j] >> shift_) & 0x3ff;
                    i16bit_target_buffer[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 12 + j] >> shift_) & 0x3ff;
                }
            }
...
bool
LMF_IO::decompress_synchronous_fADC8_signal(fADC8_signal_info_struct &signal_info, unsigned __int32 source_buffer[],
                                            unsigned __int32 source_buffer_size_in_32bit_words,
                                            __int32 &number_of_filled_16bit_words,
    ...
    if (signal_info.megasamples_per_second == 5000) {
        ...
        for (unsigned __int32 i = 4; i < limit_ + 4 - 16; i += 16) { // xxx stimmt -16?
            ...
            for (__int32 j = 0; j < 4; j++) {
                for (__int32 shift_ = 20; shift_ >= 0; shift_ -= 10) {
                    i16bit_target_buffer1[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 0 + j] >> shift_) & 0x3ff;
                    i16bit_target_buffer1[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 8 + j] >> shift_) & 0x3ff;
                    i16bit_target_buffer1[(++number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
        ...
        for (unsigned __int32 i = 4; i < limit_ + 4 - 16; i += 16) {
            ...
            for (__int32 j = 0; j < 4; j++) {
                for (__int32 shift_ = 20; shift_ >= 0; shift_ -= 10) {
                    ...
                    i16bit_target_buffer1[(number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 0 + j] >> shift_) & 0x3ff;
                    i16bit_target_buffer3[(number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 8 + j] >> shift_) & 0x3ff;
        ...
        for (unsigned __int32 i = 4; i < limit_ + 4 - 16; i += 16) {
            ...
            for (__int32 j = 0; j < 4; j++) {
                for (__int32 shift_ = 20; shift_ >= 0; shift_ -= 10) {
                    ...
                    i16bit_target_buffer1[(number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 0 + j] >> shift_) & 0x3ff;
                    i16bit_target_buffer2[(number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 4 + j] >> shift_) & 0x3ff;
                    i16bit_target_buffer3[(number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 8 + j] >> shift_) & 0x3ff;
                    i16bit_target_buffer4[(number_of_filled_16bit_words) > 0 ? number_of_filled_16bit_words : 0] =
                            (source_buffer[i + 12 + j] >> shift_) & 0x3ff;
                }
            }
